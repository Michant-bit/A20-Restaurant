...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(__('Delete Menu'), ['action' => 'delete', $menu->id], ['confirm' => __('Are you sure you want to delete {0}?', $menu->name)]) ?> </li>
        <hr>
        <li><?= $this->Html->link(__('Return'), ['action' => 'index']) ?> </li>
    </ul>
</nav>
<?php
    $this->request->session()->write('Menu.id', $menu->id);
    $this->request->session()->write('User.id', $menu->user_id);
?>
<div class="menus view large-9 medium-8 columns content">
    ...
    <div class="row">
        <h4><?= __('Details') ?></h4>
        <?= $this->Text->autoParagraph(h($menu->details)); ?>
    </div>
    <div class="related">
